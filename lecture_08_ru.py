...
def all_reduce(rank: int, world_size: int, num_elements: int):
    ...
    # Выполнять все сокращения
    # EN: Perform all-reduce
    ...
    # Измерить эффективную пропускную способность
    # EN: Measure the effective bandwidth
    dist.barrier()
...
def reduce_scatter(rank: int, world_size: int, num_elements: int):
    ...
    # Создание вводимых ресурсов и результатов
    # EN: Create input and outputs
    ...
    # Сделайте срабатывание
    # EN: Perform reduce-scatter
    start_time = time.time()
    dist.reduce_scatter_tensor(output=output, input=input, op=dist.ReduceOp.SUM, async_op=False)
    if torch.cuda.is_available():
        torch.cuda.synchronize()  # Wait for CUDA kerels to finish
...
def data_parallelism_main(rank: int, world_size: int, data: torch.Tensor, num_layers: int, num_steps: int):
    ...
    # Получение кусочков данных для этого ранга (на практике каждый званий должен загружать только свои собственные данные)
    # EN: Get the slice of data for this rank (in practice, each rank should load only its own data)
    ...
    # Создать параметры MLP parms[0], ... params[num_layers - 1] (каждое звание имеет все параметры)
    # EN: Create MLP parameters params[0], ..., params[num_layers - 1] (each rank has all parameters)
    ...
    for step in range(num_steps):
        ...
        # Синхронизация градиентов среди работников (только разница между стандартной подготовкой и НВУ)
        # EN: Sync gradients across workers (only difference between standard training and DDP)
        for param in params:
...
def tensor_parallelism_main(rank: int, world_size: int, data: torch.Tensor, num_layers: int):
    ...
    # Создать модель (каждое звание получает 1/мир_ размер параметров)
    # EN: Create model (each rank gets 1/world_size of the parameters)
    ...
    for i in range(num_layers):
        # Вычислить активацию (батч_размер x локальный_num_dim)
        # EN: Compute activations (batch_size x local_num_dim)
        x = x @ params[i]  # Note: this is only on a slice of the parameters
        x = F.gelu(x)

        # Распределить память для активаций (мир_размер x партия_ размер x локальный_num_dim)
        # EN: Allocate memory for activations (world_size x batch_size x local_num_dim)
        activations = [torch.empty(batch_size, local_num_dim, device=get_device(rank)) for _ in range(world_size)]

        # Отправить активацию через весь набор
        # EN: Send activations via all gather
        dist.all_gather(tensor_list=activations, tensor=x, async_op=False)

        # Соберите их, чтобы получить партию x num_dim
        # EN: Concatenate them to get batch_size x num_dim
        x = torch.cat(activations, dim=1)

    print(f"[tensor_parallelism] Rank {rank}: forward pass produced activations {summarize_tensor(x)}", flush=True)

    # Пропуск в обратном направлении: домашнее задание
    # EN: Backward pass: homework exercise
...
def pipeline_parallelism_main(rank: int, world_size: int, data: torch.Tensor, num_layers: int, num_micro_batches: int):
    ...
    # Каждое звание получает подмножество слоев
    # EN: Each rank gets a subset of layers
    local_params = [get_init_params(num_dim, num_dim, rank) for i in range(local_num_layers)]

    # Пропуска вперёд
    # EN: Forward pass

    # Разбиться на микропартии, чтобы свести к минимуму пузырь
    # EN: Break up into micro batches to minimize the bubble
    ...
    if rank == 0:
        ...
        # Распределить память для активации
        # EN: Allocate memory for activations
        micro_batches = [torch.empty(micro_batch_size, num_dim, device=get_device(rank)) for _ in range(num_micro_batches)]

    for x in micro_batches:
        # Получите активацию из предыдущего ранга
        # EN: Get activations from previous rank
        if rank - 1 >= 0:
            dist.recv(tensor=x, src=rank - 1)

        # Вычислить уровни, отнесенные к этому рангу
        # EN: Compute layers assigned to this rank
        for param in local_params:
            x = x @ param
            x = F.gelu(x)

        # Отправьте на следующее звание
        # EN: Send to the next rank
...
def setup(rank: int, world_size: int):
    # Указать, где находится судоводитель (уровень 0), используемая для координации (фактические данные поступают через НККЛ)
    # EN: Specify where master lives (rank 0), used to coordinate (actual data goes through NCCL)
