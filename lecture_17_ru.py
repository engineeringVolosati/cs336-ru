...
def sort_inclusion_ordering_reward(prompt: list[int], response: list[int]) -> float:  # @inspect prompt, @inspect response
    ...
    # По одному очку за каждый символ в момент, который появляется в ответе
    # EN: Give one point for each token in the prompt that shows up in the response
    inclusion_reward = sum(1 for x in prompt if x in response)  # @inspect inclusion_reward

    # Дайте по одному очку для каждой соседней пары в ответ, которая сортирована
    # EN: Give one point for each adjacent pair in response that's sorted
...
class Model(nn.Module):
    def __init__(self, vocab_size: int, embedding_dim: int, prompt_length: int, response_length: int):
        ...
        # Для каждой позиции у нас есть матрица для кодирования и матрица для декодирования
        # EN: For each position, we have a matrix for encoding and a matrix for decoding
    ...
    def forward(self, prompts: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Введи в действие позывные
        # EN: Embed the prompts
        embeddings = self.embedding(prompts)   # [batch pos dim]

        # Преобразовать с помощью матрицы быстрого положения в один вектор
        # EN: Transform using per prompt position matrix, collapse into one vector
        encoded = einsum(embeddings, self.encode_weights, "batch pos dim1, pos dim1 dim2 -> batch dim2")

        # Преобразование в один вектор на позицию реагирования
        # EN: Turn into one vector per response position
        decoded = einsum(encoded, self.decode_weights, "batch dim2, pos dim2 dim1 -> batch pos dim1")

        # Преобразование в логарифмы (вложение и выходная доля)
        # EN: Convert to logits (input and output share embeddings)
...
def generate_responses(prompts: torch.Tensor, model: Model, num_responses: int) -> torch.Tensor:
    ...
    # Пробы нумерации_реакций (независимо) для каждого [партии]
    # EN: Sample num_responses (independently) for each [batch pos]
...
def compute_log_probs(prompts: torch.Tensor, responses: torch.Tensor, model: Model) -> torch.Tensor:
    ...
    """
    # Вычислить log-оценку ответов в рамках модели
    # EN: Compute log prob of responses under model
    logits = model(prompts)  # [batch pos vocab]
    log_probs = F.log_softmax(logits, dim=-1)  # [batch pos vocab]

    # Повторяю, чтобы привести ответы в соответствие с ответами
    # EN: Replicate to align with responses
    ...
    # Индекс в log_probs с использованием ответов
    # EN: Index into log_probs using responses
...
def compute_deltas(rewards: torch.Tensor, mode: str) -> torch.Tensor:  # @inspect rewards
    ...
    if mode == "centered_rewards":
        # Вычислить среднее значение по всем ответам (испытанию) для каждой оперативной операции (батч)
        # EN: Compute mean over all the responses (trial) for each prompt (batch)
    ...
    if mode == "max_rewards":
        # Никакого вознаграждения, которое не является максимальным для каждой партии
        # EN: Zero out any reward that isn't the maximum for each batch
...
def run_policy_gradient(num_epochs: int = 100,
    ...
    # Уже убежал, просто спрячь его.
    # EN: Already ran, just cache it
    ...
    for epoch in tqdm(range(num_epochs), desc="epoch"):
        # Если использовать наказание KL, нужно получить эталонную модель (заморозить ее каждые несколько эпох)
        # EN: If using KL penalty, need to get the reference model (freeze it every few epochs)
        if kl_penalty != 0:
            if epoch % compute_ref_model_period == 0:
                ref_model = model.clone()

        # Выборочные ответы и оценка их вознаграждений
        # EN: Sample responses and evaluate their rewards
        ...
        # С учетом полученных ответов принять ряд мер
        # EN: Take a number of steps given the responses
        for step in range(num_steps_per_epoch):
            ...
            # Задние и обновленные параметры
            # EN: Backprop and update parameters
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    if use_cache:
    ...
    if use_cache:
        # Шаг по графику в сравнении с потерей и вознаграждением в двух подложках
        # EN: Plot step versus loss and reward in two subplots
