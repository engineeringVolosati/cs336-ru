...
def main():
    ...
    # Более совершенные расчеты
    # EN: More advanced computations
    triton_softmax_main()
...
def run_mlp(dim: int, num_layers: int, batch_size: int, num_steps: int) -> Callable:
    ...
    def run():
        # Запустить время модели "nume_steps" (примечание: нет обновления оптимизатора)
        # EN: Run the model `num_steps` times (note: no optimizer updates)
...
def run_operation1(dim: int, operation: Callable) -> Callable:
    # Настройка: создать одну случайную матрицу dim x dim
    # EN: Setup: create one random dim x dim matrices
    x = torch.randn(dim, dim, device=get_device())
    # Возвращать функцию для выполнения операции
    # EN: Return a function to perform the operation
    return lambda : operation(x)


def run_operation2(dim: int, operation: Callable) -> Callable:
    # Настройка: создание двух случайных матриц х dim
    # EN: Setup: create two random dim x dim matrices
    x = torch.randn(dim, dim, device=get_device())
...
def benchmark(description: str, run: Callable, num_warmups: int = 1, num_trials: int = 3):
    ...
    # Тепло: в первый раз может быть медленнее из-за компиляции, вещи не скрыты.
    # EN: Warmup: first times might be slower due to compilation, things not cached.
    # Поскольку мы прогоним ядро несколько раз, время, которое важно, стабильное.
    # EN: Since we will run the kernel multiple times, the timing that matters is steady state.
...
def profile(description: str, run: Callable, num_warmups: int = 1, with_stack: bool = False):
    ...
    # Запустить код с профайлером
    # EN: Run the code with the profiler
    with torch.profiler.profile(
            activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
            # Отметка выводной стопки для визуализации
            # EN: Output stack trace for visualization
            with_stack=with_stack,
            # Необходимо экспортировать следы для визуализации
            # EN: Needed to export stack trace for visualization
    ...
    # Текст (таблица, стенографическая форма)
    # EN: text(table, verbatim=True)

    # Записать отслеживание скопления
    # EN: Write stack trace visualization
    if with_stack:
...
def kernel_fusion_motivation():
    ...
    # Проверьте, соответствует ли реализация
    # EN: Check that the implementations match
    assert torch.allclose(y1, y2)

    # Проверять на более систематической основе
    # EN: Check more systematically
...
def create_cuda_gelu():
    ...
    # Код CUDA: имеет полную логику
    # EN: CUDA code: has the full logic
    cuda_gelu_src = open("gelu.cu").read()
    text(cuda_gelu_src, verbatim=True)

    # Код C++: определяет функцию gelu
    # EN: C++ code: defines the gelu function
...
def triton_gelu(x: torch.Tensor):
    ...
    # Распределить значения выходных значений
    # EN: Allocate output tensor
    y = torch.empty_like(x)

    # Определить сетку (элементы, разделенные на блоки)
    # EN: Determine grid (elements divided into blocks)
...
def triton_gelu_kernel(x_ptr, y_ptr, num_elements, BLOCK_SIZE: tl.constexpr):
    # Вход при x_ptr, а выход при y_ptr
    # EN: Input is at `x_ptr` and output is at `y_ptr`
    # * Блок 0 * Блок 1 * * * Блок 1*
    # EN: |        Block 0            |          Block 1          |      ...      |
    # BLOCK_SIZE num_elements
    # EN: BLOCK_SIZE                                 num_elements

    pid = tl.program_id(axis=0)
    block_start = pid * BLOCK_SIZE

    # Индексы, в которых должен работать этот ниточный блок
    # EN: Indices where this thread block should operate
    ...
    # Approx gelu = 0,5 * x * (1 + tanh( sqrt(2/pi) * (x + 0,044715 * x × β3))
    # EN: Approx gelu is 0.5 * x * (1 + tanh(sqrt(2/pi) * (x + 0.044715 * x^3)))
    # Вычислить (tl.tanh не существует, использовать tanh(a) = (exp(2a) - 1) / (exp(2a) + 1)
    # EN: Compute (tl.tanh doesn't exist, use tanh(a) = (exp(2a) - 1) / (exp(2a) + 1)
...
def manual_softmax(x: torch.Tensor):
    ...
    # EN: M: number of rows, N: number of columns
    M, N = x.shape

    # Вычислить максимум каждой строки (MN читает, M пишет)
    # EN: Compute the max of each row (MN reads, M writes)
    x_max = x.max(dim=1)[0]

    # Вычитать максимум (MN + M читает, MN пишет)
    # EN: Subtract off the max (MN + M reads, MN writes)
    x = x - x_max[:, None]

    # Экспонент (MN читает, MN пишет)
    # EN: Exponentiate (MN reads, MN writes)
    numerator = torch.exp(x)

    # Вычислить константу нормализации (MN читает, M пишет)
    # EN: Compute normalization constant (MN reads, M writes)
    denominator = numerator.sum(dim=1)

    # Нормализация (MN читает, MN пишет)
    # EN: Normalize (MN reads, MN writes)
    y = numerator / denominator[:, None]

    # Итого: 5 MN + M читает, 3MN + 2M пишет
    # EN: Total: 5MN + M reads, 3MN + 2M writes
    # В принципе, MN должен читать, MN пишет (скорость 4x!)
    # EN: In principle, should have MN reads, MN writes (speedup of 4x!)
    return y
...
def triton_softmax_kernel(x_ptr, y_ptr, x_row_stride, y_row_stride, num_cols, BLOCK_SIZE: tl.constexpr):
    ...
    # Обработать каждую строку самостоятельно
    # EN: Process each row independently
    row_idx = tl.program_id(0)
    col_offsets = tl.arange(0, BLOCK_SIZE)

    # Читать из глобальной памяти
    # EN: Read from global memory
    ...
    # EN: Write back to global memory
...
def triton_matmul_main():
    ...
    # EN: Not working for some reason
    # распечатка_ptx( "triton_matmul", тритон_matmul_kernel)
    # EN: print_ptx("triton_matmul", triton_matmul_kernel)
...
def pytorch_gelu(x: torch.Tensor):
    # Используйте приближение танх, чтобы совместить нашу реализацию
    # EN: Use the tanh approximation to match our implementation
