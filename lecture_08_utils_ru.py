...
def spawn(func: Callable, world_size: int, *args, **kwargs):
    # Примечание: Предположим, что kwargs находятся в том же порядке, что и основные потребности.
    # EN: Note: assume kwargs are in the same order as what main needs
    if sys.gettrace():
        # Если нас отслеживают, запустите функцию напрямую, так как мы не можем отследить через mp.slown
        # EN: If we're being traced, run the function directly, since we can't trace through mp.spawn
