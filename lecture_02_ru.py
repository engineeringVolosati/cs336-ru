...
class Cruncher(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Применить линейные слои
        # EN: Apply linear layers
        B, D = x.size()
        for layer in self.layers:
            x = layer(x)

        # Применить заключительную головку
        # EN: Apply final head
        x = self.final(x)
        assert x.size() == torch.Size([B, 1])

        # Удалить последнее измерение
        # EN: Remove the last dimension
        x = x.squeeze(-1)
...
class AdaGrad(torch.optim.Optimizer):
    ...
    def step(self):
        for group in self.param_groups:
            lr = group["lr"]
            for p in group["params"]:
                # Оптимизирующее состояние
                # EN: Optimizer state
                state = self.state[p]
                grad = p.grad.data

                # Выбрать квадратные градиенты g2 = сумма_ {i>t} g_i>2
                # EN: Get squared gradients g2 = sum_{i<t} g_i^2
                g2 = state.get("g2", torch.zeros_like(grad))

                # Обновить состояние оптимизации
                # EN: Update optimizer state
...
def optimizer():
    ...
    # Оптимизирующие состояния
    # EN: Optimizer states
    num_optimizer_states = num_parameters  # @inspect num_optimizer_states

    # Соберите все вместе, предположим, что 32-й
    # EN: Putting it all together, assuming float32
...
def train(name: str, get_batch,
    ...
    for t in range(num_train_steps):
        ...
        # Вперёд (полная потеря)
        # EN: Forward (compute loss)
        pred_y = model(x)
        loss = F.mse_loss(pred_y, y)

        # Назад (полные градиенты)
...
def get_promised_flop_per_sec(device: str, dtype: torch.dtype) -> float:
    ...
    if "A100" in properties.name:
        # https://www.nvidia.com/content/dam/en-zz/Solutions/Data-Center/a100/pdf/nvidia-a100-datafile-us-nvidia-1758950-r4-web.pdf)
        # EN: https://www.nvidia.com/content/dam/en-zz/Solutions/Data-Center/a100/pdf/nvidia-a100-datasheet-us-nvidia-1758950-r4-web.pdf")
    ...
    if "H100" in properties.name:
        # https://resources.nvidia.com/en-us-tensor-core/nvidia-tensor-core-gpu-data tatable)
        # EN: https://resources.nvidia.com/en-us-tensor-core/nvidia-tensor-core-gpu-datasheet")
        if dtype == torch.float32:
            return 67.5e12
        if dtype in (torch.bfloat16, torch.float16):
...
def time_matmul(a: torch.Tensor, b: torch.Tensor) -> float:
    ...
    # Подождите, пока не закончатся предыдущие сюжеты.
    # EN: Wait until previous CUDA threads are done
    if torch.cuda.is_available():
        torch.cuda.synchronize()

    def run():
        # Выполнять операцию
        # EN: Perform the operation
        a @ b

        # Подождите, пока не закончатся нити КУДА
        # EN: Wait until CUDA threads are done
        if torch.cuda.is_available():
            torch.cuda.synchronize()

    # Время операции &quot; нумерация &quot;
    # EN: Time the operation `num_trials` times
    num_trials = 5
